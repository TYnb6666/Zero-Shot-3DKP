...
from ULIP.models.pointbert.checkpoint import get_missing_parameters_message, get_unexpected_parameters_message
...
class PointTransformer_Colored(nn.Module):
    ...
    def forward(self, pts):
        ...
        concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0]], dim=-1)
        # ret = self.cls_head_finetune(concat_f)
        return concat_f, center, group_input_tokens