...
class DinoCLIP(nn.Module):
    ...
    def __init__(self, clip_backbone, class_names, vit_arch="vit_base", vit_patch_size=16, enc_type_feats="k",
        ...
        maskclip_cfg = OmegaConf.load(str(_CONFIGS_DIR / f"{clip_backbone}.yaml"))
        self.clip_backbone = build_model(maskclip_cfg["model"], class_names=class_names)  # type: ignore[arg-type]
        for param in self.clip_backbone.parameters():
            param.requires_grad = False

    def load_teachers(self):
        from models.FOUND.model import FoundModel, get_vit_encoder  # type: ignore[import-not-found]  # training-only

        _found_weights = _CONFIGS_DIR.parent / "models" / "FOUND" / "data" / "weights" / "decoder_weights.pt"
        self.found_model = FoundModel()
        assert _found_weights.is_file(), 'No weights for FOUND model'
        self.found_model.decoder_load_weights(str(_found_weights))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.apply_found:
            ...
            r_soft_found = T.functional.resize(preds, (H_feat, W_feat)).reshape(-1)  # type: ignore[attr-defined]
    ...
    def get_found_preds(self, x: torch.Tensor, resize=None):
        ...
        if resize is not None:
            preds = T.functional.resize(preds, resize)  # type: ignore[attr-defined]
        return preds
