...
class MaskClip(nn.Module):
    def __init__(
        ...
        model, _ = create_model_from_pretrained(clip_model, pretrained=pretrained)  # type: ignore[misc]
        model.eval()
        ...
        self.backbone.visual.transformer.resblocks[-2].register_forward_hook(hook_fn_forward)  # type: ignore[union-attr, index]
        self._positional_embd = nn.Parameter(self.backbone.visual.positional_embedding.data.clone())  # type: ignore[union-attr]

    @torch.no_grad()
    def extract_feat(self, inputs: Tensor) -> Tuple[Tensor]:
        """Extract features from images."""
        pos_embed = self.backbone.visual.positional_embedding  # type: ignore[union-attr]

        B, C, H, W = inputs.shape
        hw_shape = (H // self.patch_size, W // self.patch_size)
        x_len, pos_len = hw_shape[0]*hw_shape[1], pos_embed.shape[0]  # type: ignore[index]
        ...
        if x_len != pos_len:
            ...
            self.backbone.visual.positional_embedding.data = self.resize_pos_embed(  # type: ignore[union-attr]
                self._positional_embd[None], hw_shape,  (pos_h, pos_w), 'bicubic')[0]

        _ = self.backbone(inputs)
        v = self.hook_features["v"]
        v = self.extract_v(v, self.backbone.visual.transformer.resblocks[-1]).permute(1, 0, 2)  # type: ignore[union-attr, index]
        v = self.backbone.visual.ln_post(v)  # type: ignore[union-attr]
        v = v.permute(1, 0, 2)[:, 1:]
        v = v.reshape(B, hw_shape[0], hw_shape[1], -1).permute(0, 3, 1, 2).contiguous()

        self.backbone.visual.positional_embedding.data = self._positional_embd  # type: ignore[union-attr]
        return v
    ...
    def forward(self, inputs: Tensor, return_feat=False) -> Tensor:
        ...
        if return_feat:
            ...
            return seg_logits, feats  # type: ignore[return-value]
        else:
...
class MaskClipHead(nn.Module):
    def __init__(self, clip_model, class_names, in_channels=3, text_channels=512, use_templates=False, pretrained=None,
        ...
        self.proj.weight = nn.Parameter(model.visual.proj.t()[:, :, None, None])  # type: ignore[union-attr]

    @torch.no_grad()
    def update_vocab(self, class_names):
        model, _ = create_model_from_pretrained(self.clip_model, pretrained=self.pretrained)  # type: ignore[misc]
        model.eval()
    ...
    def _embed_label(self, text_model: torch.nn.Module, label: str) -> torch.Tensor:
        ...
        elif self.pretrained is not None and "laion" in self.pretrained:
            templates = ['a photo of a {}', 'a photo of an {}']
        else:
            templates = ['a {}']
        all_prompts = [self.tokenizer(template.format(label)) for template in templates]
        out = text_model.encode_text(torch.cat(all_prompts))  # type: ignore[operator]
