...
def get_config(arg=None):
  ...
  config.input = {}
  config.input.data = dict(name='imagenet2012', split='train[:98%]')  # type: ignore[attr-defined]
  ...
  config.evals.student_minival = get_eval(minival_split)  # type: ignore[attr-defined]
  config.evals.student_val = get_eval(val_split)  # type: ignore[attr-defined]
  config.evals.student_v2 = get_eval(v2_split, dataset='imagenet_v2')  # type: ignore[attr-defined]
  config.evals.student_real = get_eval(real_split, dataset='imagenet2012_real')  # type: ignore[attr-defined]
  config.evals.student_real.pp_fn = ppv.format(lbl='real_label')  # type: ignore[attr-defined]

  config.evals.student_fewshot = get_fewshot_lsr(runlocal=arg.runlocal)  # type: ignore[attr-defined]
  config.evals.student_fewshot.pred = 'student_fwd'  # type: ignore[attr-defined]
  config.evals.student_fewshot.log_steps = 10_000  # type: ignore[attr-defined]

  teacher_eval = dict(
      log_steps=100_000,  # Teacher is fixed, so rare evals.
      pred='prof_m_fwd',
  )
  config.evals.teacher_train = {**config.evals.student_train, **teacher_eval}  # type: ignore[attr-defined]
  config.evals.teacher_minival = {**config.evals.student_minival, **teacher_eval}  # type: ignore[attr-defined]
  config.evals.teacher_val = {**config.evals.student_val, **teacher_eval}  # type: ignore[attr-defined]
  config.evals.teacher_v2 = {**config.evals.student_v2, **teacher_eval}  # type: ignore[attr-defined]
  config.evals.teacher_real = {**config.evals.student_real, **teacher_eval}  # type: ignore[attr-defined]
  config.evals.teacher_fewshot = {**config.evals.student_fewshot, **teacher_eval}  # type: ignore[attr-defined]
  config.evals.teacher_fewshot.prefix = 'z_teacher/'  # type: ignore[attr-defined]
  ...
  config.evals.dist_minival = get_dist(minival_split)  # type: ignore[attr-defined]
  config.evals.dist_val = get_dist(val_split)  # type: ignore[attr-defined]
  config.evals.dist_v2 = get_dist(v2_split, dataset='imagenet_v2')  # type: ignore[attr-defined]
  ...
  config.evals.cka_train = get_cka('train[:24576]' if not arg.runlocal else 'train[:16]')  # type: ignore[attr-defined]
  config.evals.cka_minival = get_cka('train[-24576:]' if not arg.runlocal else 'train[:16]')  # type: ignore[attr-defined]
  config.evals.cka_val = get_cka('validation[:49152]' if not arg.runlocal else 'validation[:16]')  # type: ignore[attr-defined]
