...
def get_config(arg='res=512,patch_size=16'):
  ...
  config.evals.coco_panoptic_train = dict(**base, data={'split': 'train[4096:8192]'})  # type: ignore[attr-defined]
  config.evals.coco_panoptic_holdout = dict(**base, data={'split': 'train[:4096]'})  # type: ignore[attr-defined]
  config.evals.coco_panoptic = dict(**base, data={'split': 'validation'})  # type: ignore[attr-defined]
  ...
  config.evals.save_pred.pred = 'predict_panoptic'  # type: ignore[attr-defined]
  config.evals.save_pred.data = {**config.input.data}  # type: ignore[attr-defined]
