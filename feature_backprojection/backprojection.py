...
def compute_kp_dists_features(dataset: Any, feature_function: Callable[..., torch.Tensor]) -> tuple[torch.Tensor, np.ndarray[Any, np.dtype[np.floating[Any]]]]:
    """
    ...
    kp_max_id = max(
        [kp["semantic_id"] for _mesh, keypoints, _class_title, _mesh_id, _pcd in dataset[:] for kp in keypoints])

    keypoint_dists = np.zeros((kp_max_id + 1, kp_max_id + 1))
    keypoint_features_dict = {kp_id: [] for kp_id in range(kp_max_id + 1)}

    for mesh, keypoints, _class_title, _mesh_id, pcd in tqdm(dataset):
...
def features_from_views(renderer: Any, model: Any, mesh: Any, views: torch.Tensor, keypoints: list[dict[str, Any]] | None = None, only_visible: bool = True, render_dist: float = 1.0,
                        batch_size: int | None = None, device: str | torch.device = "cpu", geo_dists: np.ndarray[Any, np.dtype[np.floating[Any]]] | None = None, gaussian_sigma: float = 0.1) -> torch.Tensor:
    """
    ...
    # camera transform (PyTorch3D requires float32)
    if hasattr(views, 'dtype') and views.dtype != torch.float32:
        views = views.float()
    ...
    assert ret_array is not None
    ret_array[point_values_counts > 0] /= point_values_counts[point_values_counts > 0][:, None]
    return ret_array


def get_feature_for_pixel_location(feature_map: torch.Tensor, pixel_locations: torch.Tensor, image_size: int = 224, patch_size: int = 14, use_sam: bool = False) -> torch.Tensor:
    """
    ...
    def transform_px_to_patch_id(pixel_locations: torch.Tensor, image_size: int = 224, patch_size: int = 14) -> torch.Tensor:
        """
        :param pixel_locations: (V, N, 3) from camera.transform_points_screen
        :param image_size: Size of the rendered image
        :param patch_size: Size of the patches in the feature map
