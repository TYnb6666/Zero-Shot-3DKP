...
class ViTExtractor:
    ...
    """
    model: Any  # ViT model with patch_embed, blocks attributes
    p: int
    stride: Tuple[int, int]
    load_size: Tuple[int, int] | None
    num_patches: Tuple[int, int] | None

    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module | None = None, device: str = 'cuda'):
        """
    ...
    def create_model(model_type: str) -> Any:
        ...
        :return: the model (ViT with patch_embed, blocks attributes)
        """
        model: Any
    ...
    def patch_vit_resolution(model: nn.Module, stride: int) -> nn.Module:
        ...
        """
        patch_size = model.patch_embed.patch_size  # type: ignore[union-attr]
        if stride == patch_size:  # nothing to do
            return model

        stride = nn_utils._pair(stride)  # type: ignore[assignment]
        assert all([(patch_size // s_) * s_ == patch_size for s_ in
                    stride]), f'stride {stride} should divide patch_size {patch_size}'  # type: ignore[union-attr]

        # fix the stride
        model.patch_embed.proj.stride = stride  # type: ignore[union-attr]
        # fix the positional encoding code
        model.interpolate_pos_encoding = types.MethodType(ViTExtractor._fix_pos_enc(patch_size, stride), model)  # type: ignore[arg-type, attr-defined]
        return model

    def preprocess(self, image_path: Union[str, Path],
                   load_size: Union[int, Tuple[int, int], None] = None) -> Tuple[torch.Tensor, Image.Image]:
        """
        ...
        prep_img = prep(pil_image)[None, ...]  # type: ignore[index]
        return prep_img, pil_image

    def _get_hook(self, facet: str) -> Any:
        """
        generate a hook method for a specific block and facet.
        """
        if facet in ['attn', 'token']:
            def _hook(model: Any, input: Any, output: Any) -> None:
        ...
        def _inner_hook(module: Any, input: Any, output: Any) -> None:
            input = input[0]
            B, N, C = input.shape
    ...
    def _register_hooks(self, layers: List[int], facet: str) -> None:
        ...
        """
        for block_idx, block in enumerate(self.model.blocks):  # type: ignore[arg-type]
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] | int = 11, facet: str = 'key') -> List[torch.Tensor]:
        """
        ...
        self._feats = []
        self._register_hooks(layers, facet)  # type: ignore[arg-type]
        _ = self.model(batch)
    ...
    def _log_bin(self, x: torch.Tensor, hierarchy: int = 2) -> torch.Tensor:
        ...
        """
        assert self.num_patches is not None
        B = x.shape[0]
        ...
        for py in range(self.num_patches[0]):
            for px in range(self.num_patches[1]):
                part_idx = 0
                # fill all bins for a spatial location (py, px)
                for k in range(0, hierarchy):
                    kernel_size = 3 ** k
                    for i in range(py - kernel_size, py + kernel_size + 1, kernel_size):
                        for j in range(px - kernel_size, px + kernel_size + 1, kernel_size):
                            if i == py and j == px and k != 0:
                                continue
                            if 0 <= i < self.num_patches[0] and 0 <= j < self.num_patches[1]:
                                bin_x[:, part_idx * sub_desc_dim: (part_idx + 1) * sub_desc_dim, py, px] = avg_pools[k][
                                                                                                         :, :, i, j]
...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        image_batch, _image_pil = extractor.preprocess(args.image_path, args.load_size)
