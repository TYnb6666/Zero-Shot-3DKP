...
import numpy as np
import torch
from pytorch3d import _C  # type: ignore[attr-defined]
...
def setup_renderer(device: str | torch.device, res: int = 224, faces_per_pixel: int = 1, bin_size: int | None = None, cull_backfaces: bool = True) -> MeshRendererWithFragments:
    """
...
def sample_view_points(radius: float, partition: int) -> np.ndarray[Any, np.dtype[np.floating[Any]]]:
    """
...
def rotate_points(points: list[list[float]], angle: float, axis: list[float] | None = None) -> list[np.ndarray[Any, np.dtype[np.floating[Any]]]]:
    """
...
def check_visible_vertices(pix_to_face: torch.Tensor, mesh: Any, adjacent_faces: bool = True) -> torch.Tensor:
    """
...
def check_visible_points(pix_to_face: torch.Tensor, mesh: Any, points: torch.Tensor, adjacent_faces: bool = True) -> torch.Tensor:
    """
